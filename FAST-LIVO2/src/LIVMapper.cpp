...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<int>("map_save/interval", pcd_save_interval, -1);
  nh.param<bool>("map_save/map_save_en", pcd_save_en, false);
  nh.param<std::string>("map_save/map_type", map_type, "LAS");
  nh.param<bool>("map_save/colmap_output_en", colmap_output_en, false);
  nh.param<bool>("map_save/save_raw_points", save_raw_points, false);
  nh.param<double>("map_save/filter_size_pcd", filter_size_pcd, 0.5);
  ...
  nh.param<string>("map_save/save_dir", root_dir, "/home/viact/Data_scan/");
...
void LIVMapper::initializeComponents() 
{
  ...
  p_imu->set_gyr_bias_cov(V3D(0.01, 0.01, 0.01));
...
void LIVMapper::savePCD() 
{
  if (pcd_save_en && (pcl_wait_save->points.size() > 0 || pcl_wait_save_intensity->points.size() > 0) && pcd_save_interval < 0) 
  {
    ...
    std::string pcd_points_dir = root_dir + "/PCD/" + filename + ".pcd";

    std::string las_points_dir = root_dir + "/LAS/" + filename + ".las";
    ...
    if (img_en)
    {
      if (!save_raw_points) {
        ...
        if (map_type == "PCD") 
        {
          pcd_writer.writeBinary(pcd_points_dir, *downsampled_cloud); // Save the downsampled point cloud data
          std::cout << GREEN << "Downsampled point cloud data saved to: " << pcd_points_dir 
                    << " with point count after filtering: " << downsampled_cloud->points.size() << RESET << std::endl;
        } 
        else if (map_type == "LAS") 
        {
          saveLAS_RGB(downsampled_cloud, las_points_dir);
        }
        ...
        if (map_type == "PCD")
        {
          pcd_writer.writeBinary(pcd_points_dir, *pcl_wait_save); // Save the raw point cloud data
          std::cout << GREEN << "Raw point cloud data saved to: " << pcd_points_dir 
                    << " with point count: " << pcl_wait_save->points.size() << RESET << std::endl;
        }
        else if (map_type == "LAS") 
        {
          saveLAS_RGB(pcl_wait_save, las_points_dir);
        }
    ...
    {   
      if (map_type == "PCD")
      {   
        pcd_writer.writeBinary(pcd_points_dir, *pcl_wait_save_intensity);
        std::cout << GREEN << "Raw point cloud data saved to: " << pcd_points_dir 
                  << " with point count: " << pcl_wait_save_intensity->points.size() << RESET << std::endl;
      } else if (map_type == "LAS") 
      {
        saveLAS_Intensity(pcl_wait_save_intensity, las_points_dir);
      }
    }
  }
}

void LIVMapper::saveLAS_RGB(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, const std::string& filename)
{
    ...
    for (const auto& p : cloud->points) {
        ...
        view->setField(pdal::Dimension::Id::Red, idx, p.r);
        view->setField(pdal::Dimension::Id::Green, idx, p.g);
        view->setField(pdal::Dimension::Id::Blue, idx, p.b);
    }
    ...
    try {
      ...
      writer.execute(table, viewSet);
      std::cout << GREEN << "LAS file saved to: " << filename << RESET << std::endl;
    } catch (const pdal::pdal_error& e) {
      std::cerr << RED << "Failed to save LAS: "<< e.what() << RESET << std::endl;
    }
}

void LIVMapper::saveLAS_Intensity(pcl::PointCloud<pcl::PointXYZINormal>::Ptr cloud, const std::string& filename)
{
    pdal::PointTable table;
    auto layout = table.layout();
    layout->registerDim(pdal::Dimension::Id::X);
